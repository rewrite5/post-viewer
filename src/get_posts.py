...
#     try:
#         ID = get_data_from_username(username)["mainEntity"]["identifier"]
#         data_total = []
...
# def get_posts(username: str) -> List[Union[str, dict]]:
#     try:
#         ID = get_data_from_username(username)["mainEntity"]["identifier"]
#         data_total = []
#         url = "https://imginn.com/api/posts/"
...
async def get_posts(username: str) -> List[Union[str, dict]]:
    try:
        ID = get_data_from_username(username)["mainEntity"]["identifier"]
        data_total = []
        url = "https://imginn.com/api/posts/"
        ...
        async with httpx.AsyncClient() as client:
            while has_next:
                response = await client.get(
                    url, headers=headers, params=querystring, timeout=60
                )
                data = response.json()
                data_total.append(data)
